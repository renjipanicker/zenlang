...
/////////////////////////////////////////////////////////////////////////
inline z::Ast::QualifiedTypeSpec& z::Ast::Factory::addQualifiedTypeSpec(const z::Ast::Token& pos, const bool& isConst, const TypeSpec& typeSpec, const bool& isRef) {
    z::Ast::QualifiedTypeSpec& qualifiedTypeSpec = unit().addNode(new z::Ast::QualifiedTypeSpec(pos, isConst, typeSpec, isRef));
...
inline const z::Ast::QualifiedTypeSpec& z::Ast::Factory::getQualifiedTypeSpec(const z::Ast::Token& pos, const z::string& name) {
    ...
    const z::Ast::QualifiedTypeSpec& qTypeSpec = addQualifiedTypeSpec(pos, false, typeSpec, false);
    return qTypeSpec;
}
...
//    trace("getDef: %s %lu, unit(%lu)\n", ZenlangNameGenerator().tn(z::ref(ts), GenMode::Import).c_str(), (unsigned long)ts, z::pad(_unit));
    ...
    if(td != 0) {
        ...
        if(tdName == "pointer") {
            const z::Ast::QualifiedTypeSpec& qTypeSpec = addQualifiedTypeSpec(name, false, z::ref(td), false);
    ...
    if(ed != 0) {
        ...
        const z::Ast::QualifiedTypeSpec& qTypeSpec = addQualifiedTypeSpec(name, false, typeSpec, false);
    ...
    if(qts) {
        ...
        if((templateDefn) && (z::ref(templateDefn).name().string() == "pointer")) {
            ...
            if(mode == Unit::CoercionResult::Rhs) {
                const z::Ast::QualifiedTypeSpec& qTypeSpec = addQualifiedTypeSpec(pos, z::ref(qts).isConst(), z::ref(qts).typeSpec(), true);
        ...
        if(z::ptr(z::ref(cqts).typeSpec()) != z::ptr(initExpr.qTypeSpec().typeSpec())) {
            const z::Ast::QualifiedTypeSpec& qTypeSpec = addQualifiedTypeSpec(pos, z::ref(cqts).isConst(), z::ref(qts).typeSpec(), false);
...
inline const z::Ast::QualifiedTypeSpec& z::Ast::Factory::getFunctionReturnType(const z::Ast::Token& pos, const z::Ast::Function& function) {
    if(function.sig().outScope().isTuple()) {
        ...
        z::Ast::QualifiedTypeSpec& qTypeSpec = addQualifiedTypeSpec(pos, false, functionRetn, false);
        return qTypeSpec;
    }
...
inline z::Ast::ValueInstanceExpr& z::Ast::Factory::getValueInstanceExpr(const z::Ast::Token& pos, const z::Ast::QualifiedTypeSpec& qTypeSpec, const z::Ast::TemplateDefn& srcTemplateDefn, const z::Ast::TemplateDefn& templateDefn, const z::Ast::ExprList& exprList) {
    const z::Ast::QualifiedTypeSpec& typeSpec = addQualifiedTypeSpec(pos, qTypeSpec.isConst(), qTypeSpec.typeSpec(), true);
...
z::Ast::QualifiedTypeSpec* z::Ast::Factory::aQualifiedTypeSpec(const z::Ast::Token& pos, const bool& isConst, const z::Ast::TypeSpec& typeSpec, const bool& isRef) {
    z::Ast::QualifiedTypeSpec& qualifiedTypeSpec = addQualifiedTypeSpec(pos, isConst, typeSpec, isRef);
    return z::ptr(qualifiedTypeSpec);
}

z::Ast::QualifiedTypeSpec* z::Ast::Factory::aQualifiedTypeSpec(const bool& isConst, const z::Ast::TypeSpec& typeSpec, const bool& isRef) {
...
z::Ast::ForeachStatement* z::Ast::Factory::aEnterForeachInit(const z::Ast::Token& valName, const z::Ast::Expr& expr) {
    ...
    const z::Ast::QualifiedTypeSpec& valTypeSpec = addQualifiedTypeSpec(valName, expr.qTypeSpec().isConst(), templateDefn.at(0).typeSpec(), true);
    const z::Ast::VariableDefn& valDef = addVariableDefn(valTypeSpec, valName);
    z::Ast::Scope& scope = addScope(valName, z::Ast::ScopeType::Local);
    scope.addVariableDef(valDef);
    unit().enterScope(scope);
...
z::Ast::ForeachDictStatement* z::Ast::Factory::aEnterForeachInit(const z::Ast::Token& keyName, const z::Ast::Token& valName, const z::Ast::Expr& expr) {
    ...
    const z::Ast::QualifiedTypeSpec& keyTypeSpec = addQualifiedTypeSpec(keyName, true, templateDefn.at(0).typeSpec(), true);
    const z::Ast::QualifiedTypeSpec& valTypeSpec = addQualifiedTypeSpec(keyName, expr.qTypeSpec().isConst(), templateDefn.at(1).typeSpec(), true);
...
const z::Ast::EventDecl* z::Ast::Factory::aEnterAddEventHandler(const z::Ast::EventDecl& eventDecl) {
    z::Ast::QualifiedTypeSpec& qts = addQualifiedTypeSpec(getToken(), false, eventDecl.handler(), false);
...
z::Ast::Expr* z::Ast::Factory::aEnterFunctorCall(const z::Ast::Function& function) {
    z::Ast::QualifiedTypeSpec& qTypeSpec = addQualifiedTypeSpec(getToken(), false, function, false);
...
z::Ast::RunExpr* z::Ast::Factory::aRunExpr(const z::Ast::Token& pos, const z::Ast::FunctorCallExpr& callExpr) {
    ...
    if(function != 0) {
        z::Ast::QualifiedTypeSpec& qRetTypeSpec = addQualifiedTypeSpec(pos, false, z::ref(function), false);

        z::Ast::TemplateTypePartList& list = unit().addNode(new z::Ast::TemplateTypePartList(pos));
        list.addType(qRetTypeSpec);
        z::Ast::TemplateDefn& templateDefn = createTemplateDefn(pos, "future", list);
        const z::Ast::QualifiedTypeSpec& qTypeSpec = addQualifiedTypeSpec(pos, false, templateDefn, true);
...
z::Ast::IndexExpr* z::Ast::Factory::aIndexExpr(const z::Ast::Token& pos, const z::Ast::Expr& expr, const z::Ast::Expr& index) {
    ...
    if(td) {
        if(z::ref(td).name().string() == "list") {
            const z::Ast::QualifiedTypeSpec& qTypeSpec = addQualifiedTypeSpec(pos, z::ref(td).at(0).isConst(), z::ref(td).at(0).typeSpec(), false);
        ...
        if(z::ref(td).name().string() == "dict") {
            const z::Ast::QualifiedTypeSpec& qTypeSpec = addQualifiedTypeSpec(pos, z::ref(td).at(1).isConst(), z::ref(td).at(1).typeSpec(), true);
            z::Ast::IndexExpr& indexExpr = unit().addNode(new z::Ast::IndexExpr(pos, qTypeSpec, expr, index));
            return z::ptr(indexExpr);
        }
    }
    ...
    if(sd) {
        ...
        if(routine) {
            const z::Ast::QualifiedTypeSpec& qTypeSpec = addQualifiedTypeSpec(pos, z::ref(routine).outType().isConst(), z::ref(routine).outType().typeSpec(), true);
            z::Ast::IndexExpr& indexExpr = unit().addNode(new z::Ast::IndexExpr(pos, qTypeSpec, expr, index));
            return z::ptr(indexExpr);
        }
    }
...
z::Ast::MemberExpr* z::Ast::Factory::aMemberVariableExpr(const z::Ast::Expr& expr, const z::Ast::Token& name) {
    ...
    if(structDefn != 0) {
        for(StructBaseIterator sbi(structDefn); sbi.hasNext(); sbi.next()) {
            ...
            if(vref) {
                const z::Ast::QualifiedTypeSpec& qTypeSpec = addQualifiedTypeSpec(name, expr.qTypeSpec().isConst(), z::ref(vref).qTypeSpec().typeSpec(), true);
            ...
            for(z::Ast::StructDefn::PropertyList::const_iterator it = sbi.get().propertyList().begin(); it != sbi.get().propertyList().end(); ++it) {
                ...
                if(pref.name().string() == name.string()) {
                    const z::Ast::QualifiedTypeSpec& qTypeSpec = addQualifiedTypeSpec(name, expr.qTypeSpec().isConst(), pref.qTypeSpec().typeSpec(), true);
...
z::Ast::StructInstanceExpr* z::Ast::Factory::aStructInstanceExpr(const z::Ast::Token& pos, const z::Ast::StructDefn& structDefn, const z::Ast::StructInitPartList& list) {
    const z::Ast::QualifiedTypeSpec& qTypeSpec = addQualifiedTypeSpec(pos, false, structDefn, false);
...
z::Ast::FunctionInstanceExpr* z::Ast::Factory::aFunctionInstanceExpr(const z::Ast::Token& pos, const z::Ast::TypeSpec& typeSpec, const z::Ast::ExprList& exprList) {
    ...
    if(function != 0) {
        z::Ast::QualifiedTypeSpec& qTypeSpec = addQualifiedTypeSpec(pos, false, z::ref(function), false);
...
z::Ast::AnonymousFunctionExpr* z::Ast::Factory::aAnonymousFunctionExpr(z::Ast::ChildFunctionDefn& functionDefn, const z::Ast::CompoundStatement& compoundStatement) {
    ...
    z::Ast::ExprList& exprList = addExprList(getToken());
    z::Ast::QualifiedTypeSpec& qTypeSpec = addQualifiedTypeSpec(getToken(), false, functionDefn, false);
...
void z::Ast::Factory::initUnit() {
    z::Ast::Factory& f = z::ref(this);

    // typedef void native;
    z::Ast::TypedefDecl* tdVoid = f.aTypedefDecl(z::Ast::Token(_module.filename(), 0, 0, "void"), z::Ast::DefinitionType::Native);

    // typedef bool native;
    z::Ast::TypedefDecl* tdBool = f.aTypedefDecl(z::Ast::Token(_module.filename(), 0, 0, "bool"), z::Ast::DefinitionType::Native);

    // typedef ubyte native;
    z::Ast::TypedefDecl* tdUByte = f.aTypedefDecl(z::Ast::Token(_module.filename(), 0, 0, "ubyte"), z::Ast::DefinitionType::Native);

    // typedef ushort native;
    z::Ast::TypedefDecl* tdUShort = f.aTypedefDecl(z::Ast::Token(_module.filename(), 0, 0, "ushort"), z::Ast::DefinitionType::Native);

    // typedef uint native;
    z::Ast::TypedefDecl* tdUInt = f.aTypedefDecl(z::Ast::Token(_module.filename(), 0, 0, "uint"), z::Ast::DefinitionType::Native);

    // typedef ulong native;
    z::Ast::TypedefDecl* tdULong = f.aTypedefDecl(z::Ast::Token(_module.filename(), 0, 0, "ulong"), z::Ast::DefinitionType::Native);

    // typedef byte native;
    z::Ast::TypedefDecl* tdByte = f.aTypedefDecl(z::Ast::Token(_module.filename(), 0, 0, "byte"), z::Ast::DefinitionType::Native);

    // typedef short native;
    z::Ast::TypedefDecl* tdShort = f.aTypedefDecl(z::Ast::Token(_module.filename(), 0, 0, "short"), z::Ast::DefinitionType::Native);

    // typedef int native;
    z::Ast::TypedefDecl* tdInt = f.aTypedefDecl(z::Ast::Token(_module.filename(), 0, 0, "int"), z::Ast::DefinitionType::Native);

    // typedef long native;
    z::Ast::TypedefDecl* tdLong = f.aTypedefDecl(z::Ast::Token(_module.filename(), 0, 0, "long"), z::Ast::DefinitionType::Native);

    // typedef float native;
    z::Ast::TypedefDecl* tdFloat  = f.aTypedefDecl(z::Ast::Token(_module.filename(), 0, 0, "float"), z::Ast::DefinitionType::Native);

    // typedef double native;
    z::Ast::TypedefDecl* tdDouble = f.aTypedefDecl(z::Ast::Token(_module.filename(), 0, 0, "double"), z::Ast::DefinitionType::Native);

    z::Ast::ConstantIntExpr& cIntZero = f.aConstantIntExpr(z::Ast::Token(_module.filename(), 0, 0, "0"));
    z::Ast::ConstantBooleanExpr& cBoolFalse = f.aConstantBooleanExpr(z::Ast::Token(_module.filename(), 0, 0, "false"));
    ...
    z::Ast::CoerceList* clNum = f.aCoerceList(z::ref(tdBool));
    ...
    // typedef size native;
    z::Ast::TypedefDecl* tdSize = f.aTypedefDecl(z::Ast::Token(_module.filename(), 0, 0, "size"), z::Ast::DefinitionType::Native);

    // default size = 0;
    f.aGlobalDefaultStatement(z::ref(tdSize), cIntZero);

    // coerce int => size;
    z::Ast::CoerceList* clSize = f.aCoerceList(z::ref(tdInt));
    f.aCoerceList(z::ref(clSize), z::ref(tdSize));
    f.aGlobalCoerceStatement(z::ref(clSize));

    // typedef char native;
    z::Ast::TypedefDecl* tdChar = f.aTypedefDecl(z::Ast::Token(_module.filename(), 0, 0, "char"), z::Ast::DefinitionType::Native);

    // default char = '';
    z::Ast::ConstantCharExpr& cCharZero = f.aConstantCharExpr(z::Ast::Token(_module.filename(), 0, 0, ""));
    f.aGlobalDefaultStatement(z::ref(tdChar), cCharZero);

    // typedef string native;
    z::Ast::TypedefDecl* tdString = f.aTypedefDecl(z::Ast::Token(_module.filename(), 0, 0, "string"), z::Ast::DefinitionType::Native);

    // default string = "";
    z::Ast::ConstantStringExpr& cStringZero = f.aConstantStringExpr(z::Ast::Token(_module.filename(), 0, 0, ""));
    f.aGlobalDefaultStatement(z::ref(tdString), cStringZero);

    // coerce char => string;
    z::Ast::CoerceList* clChar = f.aCoerceList(z::ref(tdChar));
    f.aCoerceList(z::ref(clChar), z::ref(tdString));
    f.aGlobalCoerceStatement(z::ref(clChar));

    // typedef datetime native;
    z::Ast::TypedefDecl* tdDateTime = f.aTypedefDecl(z::Ast::Token(_module.filename(), 0, 0, "datetime"), z::Ast::DefinitionType::Native);

    // default datetime = 0;
    f.aGlobalDefaultStatement(z::ref(tdDateTime), cIntZero);

    // coerce int => datetime;
    z::Ast::CoerceList* clDateTime = f.aCoerceList(z::ref(tdInt));
    f.aCoerceList(z::ref(clDateTime), z::ref(tdDateTime));
    f.aGlobalCoerceStatement(z::ref(clDateTime));

    // typedef type native;
    z::Ast::TypedefDecl* tdType = f.aTypedefDecl(z::Ast::Token(_module.filename(), 0, 0, "type"), z::Ast::DefinitionType::Native);
    unused(tdType);

    // template <K> future native;
    z::Ast::TemplatePartList* tplk = f.aTemplatePartList(z::Ast::Token(_module.filename(), 0, 0, "K"));
    z::Ast::TemplateDecl* tmplFuture = f.aTemplateDecl(z::Ast::Token(_module.filename(), 0, 0, "future"), z::Ast::DefinitionType::Native, z::ref(tplk));
    unused(tmplFuture);

    // template <V> pointer native;
    z::Ast::TemplatePartList* tplv = f.aTemplatePartList(z::Ast::Token(_module.filename(), 0, 0, "V"));
    z::Ast::TemplateDecl* tmplPointer = f.aTemplateDecl(z::Ast::Token(_module.filename(), 0, 0, "pointer"), z::Ast::DefinitionType::Native, z::ref(tplv));
    unused(tmplPointer);

    // template <V> value native;
    z::Ast::TemplateDecl* tmplValue = f.aTemplateDecl(z::Ast::Token(_module.filename(), 0, 0, "value"), z::Ast::DefinitionType::Native, z::ref(tplv));
    unused(tmplValue);

    // template <V> ptr native;
    z::Ast::TemplateDecl* tmplPtr = f.aTemplateDecl(z::Ast::Token(_module.filename(), 0, 0, "ptr"), z::Ast::DefinitionType::Native, z::ref(tplv));
    unused(tmplPtr);

    // template <V> list native;
    z::Ast::TemplateDecl* tmplList = f.aTemplateDecl(z::Ast::Token(_module.filename(), 0, 0, "list"), z::Ast::DefinitionType::Native, z::ref(tplv));
    unused(tmplList);

    // template <K,V> dict native;
    z::Ast::TemplatePartList* tplkv = f.aTemplatePartList(z::Ast::Token(_module.filename(), 0, 0, "K"));
    f.aTemplatePartList(z::ref(tplkv), z::Ast::Token(_module.filename(), 0, 0, "V"));
    z::Ast::TemplateDecl* tmplDict = f.aTemplateDecl(z::Ast::Token(_module.filename(), 0, 0, "dict"), z::Ast::DefinitionType::Native, z::ref(tplkv));
    unused(tmplDict);

    // template <K,V> tree native;
    z::Ast::TemplateDecl* tmplTree = f.aTemplateDecl(z::Ast::Token(_module.filename(), 0, 0, "tree"), z::Ast::DefinitionType::Native, z::ref(tplkv));
    unused(tmplTree);

    // public routine void assert(...) native;
    const z::Ast::QualifiedTypeSpec* qtVoid = f.aQualifiedTypeSpec(z::Ast::Token(_module.filename(), 0, 0, ""), false, z::ref(tdVoid), true );
    z::Ast::RoutineDecl* rnAssert = f.aVarArgRoutineDecl(z::ref(qtVoid), z::Ast::Token(_module.filename(), 0, 0, "assert"), z::Ast::DefinitionType::Native);
    unused(rnAssert);

    // public routine void unused(...) native;
    z::Ast::RoutineDecl* rnUnused = f.aVarArgRoutineDecl(z::ref(qtVoid), z::Ast::Token(_module.filename(), 0, 0, "unused"), z::Ast::DefinitionType::Native);
    unused(rnUnused);

    // public routine void verify(...) native;
    z::Ast::RoutineDecl* rnVerify = f.aVarArgRoutineDecl(z::ref(qtVoid), z::Ast::Token(_module.filename(), 0, 0, "verify"), z::Ast::DefinitionType::Native);
    unused(rnVerify);

    // public routine void sizeof(...) native;
    z::Ast::RoutineDecl* rnSizeof = f.aVarArgRoutineDecl(z::ref(qtVoid), z::Ast::Token(_module.filename(), 0, 0, "sizeof"), z::Ast::DefinitionType::Native);
    unused(rnSizeof);

    // public routine void length(...) native;
    z::Ast::RoutineDecl* rnLength = f.aVarArgRoutineDecl(z::ref(qtVoid), z::Ast::Token(_module.filename(), 0, 0, "length"), z::Ast::DefinitionType::Native);
    unused(rnLength);

    // public function (int code)main(const list<string>& argl) abstract;
    const z::Ast::QualifiedTypeSpec* qtInt = f.aQualifiedTypeSpec(z::Ast::Token(_module.filename(), 0, 0, ""), false, z::ref(tdInt), true );
    unit().pushExpectedTypeSpec(Unit::ExpectedTypeSpec::etAssignment, z::ref(qtInt));
    const z::Ast::VariableDefn* vdMain = f.aVariableDefn(z::ref(qtInt), z::Ast::Token(_module.filename(), 0, 0, "code"));
    z::Ast::Scope* outMain = f.aParam(z::ref(vdMain));

    const z::Ast::TemplateDecl* inList = f.aTemplateTypeSpec(z::Ast::Token(_module.filename(), 0, 0, "list"));
    const z::Ast::QualifiedTypeSpec* qtString = f.aQualifiedTypeSpec(z::Ast::Token(_module.filename(), 0, 0, ""), false, z::ref(tdString), false );
    z::Ast::TemplateTypePartList* ttpl = f.aTemplateTypePartList(z::ref(qtString));
    const z::Ast::TemplateDefn* tdArgl = f.aTemplateDefnTypeSpec(z::ref(inList), z::ref(ttpl));
    const z::Ast::QualifiedTypeSpec* qtArgl = f.aQualifiedTypeSpec(z::Ast::Token(_module.filename(), 0, 0, ""), true, z::ref(tdArgl), true );
    unit().pushExpectedTypeSpec(Unit::ExpectedTypeSpec::etAssignment, z::ref(qtArgl));
    const z::Ast::VariableDefn* vdArgl = f.aVariableDefn(z::ref(qtArgl), z::Ast::Token(_module.filename(), 0, 0, "argl"));
    z::Ast::Scope* inMain = f.aParam(z::ref(vdArgl));

    z::Ast::FunctionSig* mainSig = f.aFunctionSig(z::ref(outMain), z::Ast::Token(_module.filename(), 0, 0, "main"), z::ref(inMain));
    z::Ast::ClosureRef cref = f.aClosureList();
    z::Ast::RootFunctionDecl* mainDecl = f.aRootFunctionDecl(z::ref(mainSig), z::Ast::DefinitionType::Native, cref);
    unused(mainDecl);

    //public function (int passed)test() abstract;
    unit().pushExpectedTypeSpec(Unit::ExpectedTypeSpec::etAssignment, z::ref(qtInt));
    const z::Ast::VariableDefn* vdTest = f.aVariableDefn(z::ref(qtInt), z::Ast::Token(_module.filename(), 0, 0, "passed"));
    z::Ast::Scope* outTest = f.aParam(z::ref(vdTest));
    z::Ast::Scope* inTest = f.aParam(z::Ast::ScopeType::Param);
    z::Ast::FunctionSig* testSig = f.aFunctionSig(z::ref(outTest), z::Ast::Token(_module.filename(), 0, 0, "test"), z::ref(inTest));
    z::Ast::RootFunctionDecl* testDecl = f.aRootFunctionDecl(z::ref(testSig), z::Ast::DefinitionType::Native, cref);
    unused(testDecl);
}
