...
struct vfsc_file {
  ...
  FILE* hFile;             /* The underlying file handle */
};
...
*/
static FILE* OpenSparseFile(const char *zName)
{
    FILE* hSparseFile;
#if SQLITE_OS_WIN
    ...
    hSparseFile = fopen((WCHAR*)zConverted, "rw");
...
*/
static int IsCompressed(FILE* hFile)
{
    char buffer[100];
    int read = 0;
//    long upper = 0;
...
//      vfsc_printf(pInfo, OpenClose, "> %s.xOpen(%s) -> %x", pInfo->zVfsName, p->zFName, GetLastError());
      vfsc_printf(pInfo, OpenClose, "> %s.xOpen(%s) -> %x", pInfo->zVfsName, p->zFName, 0);
      if (p->hFile != INVALID_HANDLE_VALUE)
      {
