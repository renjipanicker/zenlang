...
namespace {
    ...
    private:
        ...
        virtual void visit(const Ast::TypeofTypeExpr& node) {
            fprintf(_fp, "z::type(\"%s\")", getQualifiedTypeSpecName(node.typeSpec(), GenMode::Import).c_str());
        }
        ...
        virtual void visit(const Ast::TypeofExprExpr& node) {
            ...
            if(td) {
                ...
                fprintf(_fp, "z::type(\"%s\")", getQualifiedTypeSpecName(node.expr().qTypeSpec(), GenMode::Import).c_str());
            }
        }
        ...
        virtual void visit(const Ast::DynamicTypecastExpr& node) {
            ExprGenerator(_fp).visitNode(node.expr());
            fprintf(_fp, ".getT<%s>()", getTypeSpecName(node.qTypeSpec().typeSpec(), GenMode::Import).c_str());
        }
        ...
        virtual void visit(const Ast::StructInstanceExpr& node) {
            ...
            for(Ast::StructInitPartList::List::const_iterator it = node.list().list().begin(); it != node.list().list().end(); ++it) {
                ...
                fprintf(_fp, "._%s<%s>(", part.vdef().name().text(), getTypeSpecName(node.structDefn(), GenMode::Import).c_str());
                visitNode(part.expr());
                fprintf(_fp, ")");
            }
        }
