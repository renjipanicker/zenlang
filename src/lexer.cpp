...
inline void Lexer::Impl::dump(const std::string& x) const {
    printf("%s: buffer %lu, bufferEnd %lu, start %lu, marker %lu, cursor %lu, limit %lu, limit-cursor %ld, text '%s'\n",
...
void Lexer::Impl::push(Ast::NodeFactory& factory, const char* input, size_t inputSize, const bool& isEof) {
    printf("push(0): isEof %d, inputSize %lu, input '%s'\n", isEof, inputSize, input);
    ...
    dump("push(1)");
    printf("maxFill %lu, required %lu, used %lu, needed %lu, allocated %lu\n", maxFill, required, used, needed, allocated);
    ...
    printf("consumed %lu, maxFill %lu, *cursor %d, *cursor %c\n", consumed, maxFill, *_cursor, *_cursor);
    if(consumed > 0) {
